...
const FIGURE_IMAGE_BASE = 'http://38.246.237.84:32770/image';
...
const ProgressBar = ({
  ...
  return (
    <div className="min-w-0 space-y-0.5">
      <div className="flex min-w-0 justify-between items-center gap-1.5 text-[10px] uppercase tracking-widest text-white/55">
...
const Field = ({k, v}: {k: string; v?: React.ReactNode}) => (
  <div className="grid min-w-0 grid-cols-1 gap-1 py-1.5 border-b border-white/5 last:border-b-0 sm:grid-cols-[minmax(4.5rem,5.75rem)_minmax(0,1fr)] sm:items-start sm:gap-2.5">
    <div className="text-[10px] text-white/40 tracking-widest uppercase font-semibold shrink-0 sm:pt-0.5">{k}</div>
    <div className="min-w-0 text-[12px] text-white/80 leading-snug wrap-break-word">{v ?? <span className="text-white/30">—</span>}</div>
  </div>
);
...
const Card = ({
  ...
  <section className="bg-black/40 rounded-xl border border-white/10 overflow-hidden">
    <div className="px-4 py-2.5 flex items-center gap-2 border-b border-white/10 bg-white/5">
      {Icon ? <Icon size={14} className="text-orange-400" /> : null}
      <div className="text-[12px] font-extrabold tracking-[0.22em] text-white/90">{title}</div>
    </div>
...
function FigureGalleryPathList({urls}: {urls: string[]}) {
  ...
  return (
    <div className="rounded-lg border border-white/12 bg-black/45">
      <div className="border-b border-white/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-white/50">
        请按以下完整路径放置图片（每行一条，共 {urls.length} 张）
      </div>
      <div className="max-h-[min(38vh,280px)] overflow-y-auto overscroll-y-contain px-2 py-1.5 [-webkit-overflow-scrolling:touch] sm:max-h-[320px]">
  ...
  if (!url) {
    ...
    return (
      <Card title="云端图库" icon={ImageIcon}>
        <div className="space-y-2.5 text-[12px] leading-relaxed text-white/55">
    ...
    <Card title="云端图库" icon={ImageIcon}>
      <div className="space-y-3">
        {!failed ? (
          ...
          <div className="space-y-3">
            <p className="text-center text-[12px] leading-snug text-white/50">当前图片未加载，请确认下列路径上文件是否存在（HTTPS 页面也可能拦截 HTTP 图片）。</p>
      ...
      <div className="flex h-full min-h-0 w-full flex-1 flex-col overflow-hidden px-2 sm:px-3 lg:px-4">
        <header className="shrink-0 px-2 sm:px-3 pt-2 sm:pt-2.5 pb-1">
          <div className="rounded-xl border border-[#8a552f]/55 bg-[linear-gradient(90deg,rgba(84,46,24,0.9),rgba(41,23,15,0.92)_56%,rgba(20,12,10,0.94))] px-3 py-1.5 sm:px-3 sm:py-2 shadow-[inset_0_1px_0_rgba(255,220,180,0.05),0_10px_28px_rgba(0,0,0,0.24)]">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              ...
              <div className="whitespace-nowrap text-[13px] font-extrabold tracking-[0.22em] text-[#ffd29a] sm:text-[15px] sm:tracking-[0.28em]">
                温泉·实况监测
              </div>
            </div>
          </div>
        ...
        <main className="warm-spring-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-y-contain px-2 pb-2 pt-0 sm:px-3 sm:pb-4 [-webkit-overflow-scrolling:touch] space-y-2.5">
          ...
          <div className="hidden md:grid md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)] md:gap-2.5 md:items-stretch">
            <div className="grid min-w-0 gap-2.5 self-stretch h-full min-w-[0]">
              <section className="min-w-0 rounded-xl border border-[#5a3720]/62 overflow-hidden bg-[linear-gradient(180deg,rgba(53,31,18,0.95),rgba(24,14,10,0.98)_44%,rgba(8,6,7,0.99))]">
                ...
                <div className="p-1 h-full">
                  <div className="min-w-0 h-full flex flex-col justify-start space-y-1.5">
              ...
              <div className="min-w-0 rounded-xl border border-[#5a3720]/62 overflow-hidden bg-[linear-gradient(180deg,rgba(53,31,18,0.95),rgba(24,14,10,0.98)_44%,rgba(8,6,7,0.99))]">
                <div className="flex min-w-0 shrink-0 flex-nowrap items-center justify-between gap-2 border-b border-[#c06d2e]/38 bg-[linear-gradient(90deg,rgba(132,76,38,0.52),rgba(62,36,22,0.72),rgba(26,15,10,0.92))] px-4 py-1.5">
                  <div className="flex min-w-0 items-center gap-2">
                    <Users size={14} className="shrink-0 text-white" />
                    <div className="whitespace-nowrap text-[11px] font-black tracking-widest text-white">焦点旅客</div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setDetailOpen(true)}
                    className="shrink-0 whitespace-nowrap rounded-md border border-white/22 bg-black/70 px-2 py-0.5 text-[11px] font-bold leading-none text-white backdrop-blur-sm transition hover:bg-black/82 active:scale-[0.99]"
                  >
                    查看详情
                  </button>
                </div>
                <div className="p-1 h-full">
                  <div className="min-w-0 h-full flex flex-col justify-start space-y-1.5">
                    ...
                    <Field k="位置" v={data.焦点旅客?.位置 ?? '位置未明'} />
            ...
            <div className="grid min-w-0 gap-2 min-h-0 self-start">
              <Card title="旅客B" icon={Users}>
                <div className="min-w-0 space-y-0.5">
                  ...
                  <Field
                    ...
                    v={
                      data.旅客B?.近况?.trim() ? (
                        <span className="block whitespace-pre-wrap break-words leading-snug text-slate-200/95">
                          {data.旅客B.近况}
                        </span>
                      ) : undefined
                    }
              ...
              <Card title="环境" icon={MapPin}>
                <div className="space-y-2">
                  ...
                  <div>
                    <div className="text-[10px] text-white/40 tracking-widest uppercase font-semibold mb-1">周边视线</div>
                    <TagList items={sight} empty="—" />
                  </div>
                  <div>
                    <div className="text-[10px] text-white/40 tracking-widest uppercase font-semibold mb-1">冴子处理</div>
          ...
          <div className="md:hidden">
            <section className="rounded-xl border border-[#5a3720]/62 overflow-hidden bg-[linear-gradient(180deg,rgba(53,31,18,0.95),rgba(24,14,10,0.98)_44%,rgba(8,6,7,0.99))]">
              <div className="px-4 py-2.5 border-b border-[#c06d2e]/32">
                <div className="text-[12px] font-extrabold tracking-[0.22em] text-white">主角 · 焦点旅客</div>
              </div>
              <div className="p-3.5 max-sm:p-4">
      ...
      {detailOpen ? (
        <div className="fixed inset-0 z-50 bg-[rgba(0,0,0,0.78)] backdrop-blur-md flex items-end sm:items-center justify-center p-2 sm:p-4">
          <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-t-3xl border border-[#5a3822]/42 bg-[linear-gradient(180deg,rgba(26,16,12,0.98),rgba(12,8,7,0.99)_44%,rgba(4,3,4,1))] shadow-[0_30px_90px_rgba(0,0,0,0.76)] sm:rounded-3xl">
            <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-[#8a552f]/28 bg-[linear-gradient(90deg,rgba(116,68,32,0.42),rgba(61,36,22,0.22),rgba(255,255,255,0.03))] shrink-0">
              <div>
