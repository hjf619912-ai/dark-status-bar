...
function getStyleIcon(style: string): string {
    ...
    const found = DMC_STYLES.find(
        (x) => x.cn === s || (x.cn === '皇家护卫' && (s === '皇家护卫' || s === '皇家守卫'))
    );
    return found?.icon ?? '';
}
...
function getRuntimeMvuData<T>(
    ...
    const currentMessageId =
        typeof g.getCurrentMessageId === 'function' ? g.getCurrentMessageId() : 'latest';
    ...
    const messageCandidates: Array<string | number> =
        mode === 'latest_message'
            ? ['latest', currentMessageId]
            : [currentMessageId, 'latest'];
        ...
        if (g.Mvu && typeof g.Mvu.getMvuData === 'function') {
            ...
            for (const messageId of messageCandidates) {
                ...
                if (mvuData && typeof mvuData === 'object' && Object.keys(mvuData).length > 0) {
                    ...
                    const candidateData =
                        statData && typeof statData === 'object' ? (statData as T) : (mvuData as T);
...
function getDualSourceSnapshot() {
    ...
    return {
        ...
        messageLatest: {
            ...
            行动预判选项数: Array.isArray((latestData as any)?.行动预判?.选项)
                ? (latestData as any).行动预判.选项.length
                : 0,
        },
        chat: {
            当前互动角色键: Object.keys((chatData as any)?.当前互动角色 || {}),
            追随者与猎物键: Object.keys((chatData as any)?.追随者与猎物 || {}),
            行动预判选项数: Array.isArray((chatData as any)?.行动预判?.选项)
                ? (chatData as any).行动预判.选项.length
                : 0,
        },
    };
}
...
function hasRuntimeBridge() {
    ...
    const hasReplace = typeof g.replaceVariables === 'function' || !!(g.Mvu && typeof g.Mvu.replaceMvuData === 'function');
...
export default function App() {
    ...
    const [companionAvatarText, setCompanionAvatarText] = useState(JSON.stringify(settings.companionAvatarPaths, null, 2));
    ...
    const getVeilMatchChain = (name: string, status: string, profile?: { gender?: string; job?: string; race?: string }) => {
    ...
    const getRandomVeilImage = (
        ...
        if (veilRule) {
            const total = Math.max(
                1,
                Number(veilRule.statusCounts?.[status] ?? veilRule.defaultCount ?? 1),
            );
            const picked = Math.floor(Math.random() * total) + 1;
    ...
    const textActionSource =
        typeof rawActionNode === 'string'
            ? rawActionNode
            : typeof heroActionNode === 'string'
                ? heroActionNode
                : '';
    ...
    const currentCompanionPreview = companion
        ? (() => {
            ...
            return {
                ...
                头像命中链路: getAvatarMatchChain(companion.name, {
                    gender: companion.gender,
                    job: companion.job,
                    race: companion.race,
                }, true),
    ...
    const currentEncounterPreview = selectedEncounter
        ? (() => {
            ...
            return {
                ...
                命中规则: matched?.key || '未命中',
                头像命中链路: getAvatarMatchChain(target.name, {
                    gender: target.gender,
                    job: target.job,
                    race: target.race,
                }, false),
    ...
    return (
        <div className="dashboard" id="main-dashboard">
            ...
            <nav className="nav">
                ...
                <div 
                    className={`nav-item ${activeTab === 'world' ? 'active' : ''}`}
            ...
            <main className="content">
                        ...
                        <div className="hero-profile">
                            ...
                            <div className="hero-avatar">
                                {heroAvatar ? (
                                    ...
                                    <img src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=200" alt="Hero" referrerPolicy="no-referrer" />
                                )}
                            </div>
                        ...
                        <div className="status-bars">
                            <div className="bar-container">
                                <div className="bar-label">
                                    <span>HP 生命值</span>
                                    <span>{hero.hpCurrent}/{hero.hpMax}</span>
                                </div>
                            ...
                            <div className="bar-container">
                                <div className="bar-label">
                                    <span>MP 魔力值</span>
                                    <span>{hero.mpCurrent}/{hero.mpMax}</span>
                                </div>
                                <div className="bar-bg">
                        ...
                        {companions.length > 0 && (
                            <div className="companion-selector">
                                {companions.map((comp, index) => (
                                    ...
                                    >
                                        <img
                                            src={getCompanionAvatar(comp.name, { gender: comp.gender, job: comp.job, race: comp.race }) || "https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?auto=format&fit=crop&q=80&w=600"}
                                            alt={comp.name}
                        ...
                        {companions.length > 0 && companion && (
                            <div className="companion-detail">
                                <div className="companion-avatar-large">
                                    ...
                                    <img src={getCompanionAvatar(companion.name, { gender: companion.gender, job: companion.job, race: companion.race }) || "https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?auto=format&fit=crop&q=80&w=600"} alt={companion.name} referrerPolicy="no-referrer" />
                                </div>
                                <h3 className="companion-name">{companion.name}</h3>
                                <div className="companion-status">{companion.race} · {companion.role} · {companion.status}</div>
                                ...
                                <div className="bar-container" style={{ width: '100%' }}>
                                    ...
                                    <div className="bar-bg">
                                        <div className="bar-fill" style={{ width: `${companion.love}%`, background: 'linear-gradient(90deg, #be185d, #ec4899)' }}></div>
                                    </div>
                                </div>
                                ...
                                <div className="bar-container" style={{ width: '100%' }}>
                                    ...
                                    <div className="bar-bg">
                                        <div className="bar-fill" style={{ width: `${companion.obedience}%`, background: 'linear-gradient(90deg, #9d174d, #f472b6)' }}></div>
                                    </div>
                                </div>
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            if (!getEffectiveVeilBaseUrl()) {
                                                notifyVeil(
                                                    '请先填写「暗夜面纱基础 URL」并保存设置，或在 MVU 中设置 暗夜面纱.基础URL',
                                                );
                                                return;
                                            }
                ...
                <div className={`tab-content ${activeTab === 'encounters' ? 'active' : ''}`} style={{ width: '100%' }}>
                    {encounters.length > 0 && selectedEncounter && (
                        <div className="companion-detail" style={{ color: 'var(--pink)', flex: 1, justifyContent: 'center', width: '100%' }}>
                            <div className="companion-selector">
                                {encounters.map((enc, index) => (
                                    ...
                                    >
                                        <img
                                            src={getNpcAvatar(enc.name, { gender: enc.gender, job: enc.job, race: enc.race }) || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=600"}
                                            alt={enc.name}
                                            referrerPolicy="no-referrer"
                                        />
                                    </div>
                            ...
                            <div className="companion-avatar-large">
                                <div className="avatar-glow"></div>
                                <img
                                    src={getNpcAvatar(selectedEncounter.name, { gender: selectedEncounter.gender, job: selectedEncounter.job, race: selectedEncounter.race }) || "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&q=80&w=600"}
                                    alt={selectedEncounter.name}
                                    referrerPolicy="no-referrer"
                                />
                            </div>
                            <h3 className="companion-name" style={{ color: 'var(--text-main)' }}>{selectedEncounter.name}</h3>
                            <div className="companion-status" style={{ opacity: 0.8 }}>{selectedEncounter.race} · {selectedEncounter.status}</div>
                    ...
                    {encounters.length === 0 && (
                        <div className="companion-detail" style={{ color: 'var(--pink)', flex: 1, justifyContent: 'center', width: '100%' }}>
                            <h3 className="companion-name" style={{ color: 'var(--text-main)' }}>暂无邂逅角色</h3>
                            <div className="companion-status" style={{ opacity: 0.8 }}>`当前互动角色` 当前为空</div>
                        </div>
                    )}
                </div>
                ...
                <div className={`tab-content ${activeTab === 'world' ? 'active' : ''}`} style={{ width: '100%' }}>
                    ...
                    <div style={{ marginTop: 'auto', paddingTop: '20px', width: '100%' }}>
                        <div style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid var(--glass-border)', width: '100%' }}>
                            <h4 style={{ fontSize: '0.8rem', color: 'var(--gold)', marginBottom: '8px', textTransform: 'uppercase' }}>当前状况总结</h4>
            ...
            {createPortal(
                <div className={`modal ${isSettingsOpen ? 'active' : ''}`} onClick={() => setIsSettingsOpen(false)}>
                    <div className="modal-content settings-modal" onClick={(e) => e.stopPropagation()}>
                        <span className="modal-close" onClick={() => setIsSettingsOpen(false)}>&times;</span>
                    ...
                    {!isSettingsUnlocked && (
                        <div className="settings-row">
                            ...
                            />
                            <button className="btn-pink" onClick={unlockSettings}>解锁设置</button>
                        </div>
                    )}
                        ...
                        <>
                            ...
                            <div className="settings-row">
                                ...
                                <div className="settings-inline">
                                    ...
                                    <button className="btn-pink btn-inline" onClick={retryRuntimeBridge}>重试桥接</button>
                                </div>
                            </div>
                            ...
                            <div className="settings-guide-box">
                                <strong>小白指路 · 头像与面纱</strong><br />
                                ① <strong>队友</strong>：追随者与猎物里的固定队友，按姓名映射<br />
                                ② <strong>通用 NPC</strong>：已知姓名的 NPC（当前互动角色等），按姓名映射，优先于随机规则<br />
                                ③ <strong>随机 NPC 规则</strong>：未在①②里配置的 NPC，按「性别|职业|种族」匹配，键支持 * 通配；avatar 控制头像，veil 控制面纱图<br />
                                ④ <strong>图库计数</strong>：暗夜面纱随机抽图用，记录「角色名→状态→该状态图片数量」
                            </div>
                            ...
                            <div className="settings-row">
                                ...
                                <span className="settings-hint">已知姓名的 NPC（当前互动角色等），按姓名映射；命中则不再走随机规则</span>
                                <textarea
                                    value={npcAvatarText}
                            ...
                            <div className="settings-row">
                                ...
                                <span className="settings-hint">键示例：女|老板娘|半魅魔 —— 未在「通用 NPC」里的 NPC 按此匹配；avatar 配头像，veil 配面纱</span>
                                <textarea
                                    value={randomNpcRuleText}
                                    onChange={(e) => setRandomNpcRuleText(e.target.value)}
                                    placeholder={'{"女|老板娘|半魅魔/半人类混血":{"avatar":{"path":"npc/random/tavern_owner","count":8},"veil":{"path":"veil/random/tavern_owner","statusCounts":{"微醺":6,"日常":3},"defaultCount":2}},"*|战士|兽人":{"avatar":{"path":"npc/random/orc_warrior","count":10}}}  // 或写入变量键: 随机NPC资源规则'}
                                />
                            </div>
                            <div className="settings-row">
                                <label>快速新增随机 NPC 规则</label>
                                <span className="settings-hint">填写下方 性别|职业|种族、头像数量、状态名及数量后，点击按钮自动写入上方的 JSON</span>
                            ...
                            <div className="settings-row">
                                ...
                                <span className="settings-hint">记录每个角色、每种状态下的图片数量，用于随机抽图；可与 MVU 暗夜面纱.图库计数 同步</span>
                                <textarea
                            ...
                            <div className="settings-row">
                                ...
                                <button className="btn-pink" onClick={addQuickGalleryRule}>添加到图库计数 JSON</button>
                            </div>
                            ...
                            <div className="settings-row">
                                ...
                                <input
                                    ...
                                    placeholder="再次输入新密码"
                                />
                                <button className="btn-pink" onClick={changePassword}>更新密码</button>
                            </div>

                            <button className="btn-pink" onClick={saveSettings}>保存设置</button>
                        </>
                    )}
