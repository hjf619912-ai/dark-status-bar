...
function getChanceTone(pct: number): string {
    // 分段成功率颜色：醒目但不刺眼
    // 1~30 灰色，30~60 琥珀，60~80 绿色，80~100 青色
    const p = Math.max(0, Math.min(100, Math.round(pct)));
    if (p <= 30) return '#94a3b8';
...
function getRuntimeMvuData<T>(
  ...
  const mergeRootWithStatData = (root: any, statData: any) => {
    ...
    // Important: MVU sometimes stores fields on the root object (outside stat_data).
    // We merge them so UI can still see those fields (e.g. 主角.行动预判) while keeping stat_data as the source of truth.
    const merged = { ...root, ...statData };
    if (root.世界 || statData.世界) merged.世界 = { ...(root.世界 || {}), ...(statData.世界 || {}) };
    if (root.主角 || statData.主角) merged.主角 = { ...(root.主角 || {}), ...(statData.主角 || {}) };
    if (root.暗夜面纱 || statData.暗夜面纱) merged.暗夜面纱 = { ...(root.暗夜面纱 || {}), ...(statData.暗夜面纱 || {}) };
    return merged;
  };
  const tryChatFirst = () => {
    try {
        ...
        if (mvuData && typeof mvuData === 'object' && Object.keys(mvuData).length > 0) {
          ...
          const candidateData =
            statData && typeof statData === 'object'
              ? (mergeRootWithStatData(mvuData, statData) as T)
              : (mvuData as T);
          const candidateSource =
            statData && typeof statData === 'object'
              ? `Mvu.getMvuData(message.${String(messageId)}).stat_data(+root)`
  ...
  const readMessage = (messageId: string | number) => {
    try {
      if (g.Mvu && typeof g.Mvu.getMvuData === 'function') {
        ...
        const statData = (m as any).stat_data;
        if (statData && typeof statData === 'object') {
          const merged = { ...m, ...statData, 主角: { ...(m as any).主角, ...(statData as any).主角 } };
          return sanitizeRuntimeData(merged);
        }
        return sanitizeRuntimeData(m);
      }
      if (typeof g.getVariables === 'function') {
...
export default function App() {
  ...
  // 当运行时没有行动预判选项时，在 UI 上显示一个固定兜底选项
  // 说明：这只影响“显示/可点”，不写回酒馆变量，因此不违背你“关闭时不输出行动预判”的要求。
  ...
  const hunterLevelRaw = (mvuData as any)?.世界?.猎人等级;
  const hunterLevel = String(hunterLevelRaw || 'D').toUpperCase();
  const hunterTheme = (() => {
    const l = hunterLevel;
    const themes: Record<string, { accent: string; glow: string; gradient: string }> = {
      D: { accent: '#94a3b8', glow: 'rgba(148,163,184,0.45)', gradient: 'linear-gradient(135deg, rgba(148,163,184,0.35), rgba(148,163,184,0.08))' },
      C: { accent: '#22c55e', glow: 'rgba(34,197,94,0.45)', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.35), rgba(34,197,94,0.08))' },
      B: { accent: '#0ea5e9', glow: 'rgba(14,165,233,0.48)', gradient: 'linear-gradient(135deg, rgba(14,165,233,0.35), rgba(14,165,233,0.08))' },
      A: { accent: '#a855f7', glow: 'rgba(168,85,247,0.48)', gradient: 'linear-gradient(135deg, rgba(168,85,247,0.35), rgba(168,85,247,0.08))' },
      S: { accent: '#fbbf24', glow: 'rgba(251,191,36,0.52)', gradient: 'linear-gradient(135deg, rgba(251,191,36,0.38), rgba(251,191,36,0.09))' },
      SS: { accent: '#ef4444', glow: 'rgba(239,68,68,0.52)', gradient: 'linear-gradient(135deg, rgba(239,68,68,0.38), rgba(239,68,68,0.09))' },
    };
    return themes[l] ?? themes.D;
  })();

  // 战斗风格进度（用于红魔石/特殊怪物的成长展示）
  // 默认从 MVU/变量的：主角.风格进度 读取，形如 { "剑圣": 0, "枪神": 30, "骗术师": 60, "皇家护卫": 10 }
  const styleProgressRaw = ((mvuData as any)?.主角?.风格进度 || {}) as Record<string, unknown>;
  const clamp01to100 = (n: unknown) => {
    const v = Number(n);
    if (!Number.isFinite(v)) return 0;
    return Math.max(0, Math.min(100, Math.round(v)));
  };
  const getStyleProgress = (cn: string) => {
    // 允许少量兼容：cn 本身、cn+"进度" 等
    return clamp01to100(styleProgressRaw?.[cn] ?? styleProgressRaw?.[`${cn}进度`] ?? 0);
  };
  const getStyleProgressGradient = (cn: string) => {
    // 醒目但不刺眼：金/琥珀/绿/青
    switch (cn) {
      case '剑圣':
      ...
      case '骗术师':
        return 'linear-gradient(90deg, #a855f7, #22d3ee)';
      case '皇家护卫':
  ...
  // 竖向进度条渐变
  const getStyleProgressGradientVertical = (cn: string) => {
    switch (cn) {
      case '剑圣':
        return 'linear-gradient(180deg, #fbbf24, #f59e0b)';
      case '枪神':
        return 'linear-gradient(180deg, #22d3ee, #0ea5e9)';
      case '骗术师':
        return 'linear-gradient(180deg, #22d3ee, #a855f7)';
      case '皇家护卫':
        return 'linear-gradient(180deg, #ef4444, #f97316)';
      default:
        return 'linear-gradient(180deg, #fbbf24, #f59e0b)';
    }
  };
  ...
  // 当前委托（用于 UI 展示：委托等级 + 地点 + 怪物属性）
  const currentQuestSummary = (() => {
    const grade = (mvuData?.世界?.当前委托等级 || 'D') as string;
    const loc = (mvuData?.世界?.当前委托地点 || '山') as string;

    const locDesc: Record<string, string> = {
      山: '近战类性',
      泽: '深潜与水魔力',
      空: '飞行',
      炎: '坚硬灼热外表',
    ...
    const gradeDesc: Record<string, string> = {
      D: 'D级：最终阶段30%精英，其余寻常怪物',
      C: 'C级：最终阶段70%精英，其余寻常怪物',
      B: 'B级：必有精英 + 30%领主怪物',
      A: 'A级：必有精英 + 领主怪物',
      EX: 'EX：论外不可观测，极危险；死亡必生成魔具',
    };

    const locTag = locDesc[loc] ?? '未知属性';
    const gradeTag = gradeDesc[grade] ?? '未知等级危险度';

    return `等级：${grade}｜地点：${loc}｜怪物属性：${locTag}｜${gradeTag}`;
  })();

  // 在世界 tab 下多一张「当前委托」
  const worldWithQuest = [
    ...world,
    { label: '当前委托', value: currentQuestSummary, icon: 'ph-target' },
  ];
  // 行动预判是本次最核心的数据之一。
  // 为了 100% 对齐“你控制台直接 Mvu.getMvuData(message).stat_data 解析出来的结果”，
  // 这里对行动预判单独走一遍“从 MVU 直接取值 + root/stat_data 合并”的逻辑，
  // 避免运行时合并/选择 message_id 的过程让 stat_data 字段丢失。
  const heroActionNode = (() => {
    const g = globalThis as any;
    const getMid = () => (typeof g.getCurrentMessageId === 'function' ? g.getCurrentMessageId() : 'latest');
    const candidates: Array<string | number> = [getMid(), 'latest'];
    let emptyOptionsNode: any = null;
    let stringActionNode: any = null;

    if (g.Mvu && typeof g.Mvu.getMvuData === 'function') {
      for (const messageId of candidates) {
        const raw = g.Mvu.getMvuData({ type: 'message', message_id: messageId }) || {};
        if (raw && typeof raw === 'object') {
          const statData = (raw as any).stat_data;
          const merged =
            statData && typeof statData === 'object'
              ? { ...raw, ...statData, 主角: { ...((raw as any).主角 || {}), ...((statData as any).主角 || {}) } }
              : raw;

          const node = (merged as any)?.主角?.行动预判 ?? (merged as any)?.行动预判;
          if (node && typeof node === 'object' && Array.isArray((node as any).选项)) {
            // 优先取“非空选项”。
            // 这能避免 current message 恰好为空、而 latest message 有 4 项时被错误截断。
            if ((node as any).选项.length > 0) {
              return node;
            }
            // 先记住空数组候选；只有所有候选都没非空时才回退到它。
            if (!emptyOptionsNode) {
              emptyOptionsNode = node;
            }
            continue;
          }
          if (typeof node === 'string') {
            // string 默认项：不要立刻返回，避免“当前楼层为空/默认字符串”，抢占了 latest 的 options 对象。
            if (!stringActionNode) {
              stringActionNode = node;
            }
            continue;
          }
        }
      ...
      if (emptyOptionsNode) {
        return emptyOptionsNode;
      }
      if (stringActionNode) {
        return stringActionNode;
      }
    }

    // 最后兜底：使用已合并后的 mvuData（避免某些环境下 MVU 未就绪导致空白）
    return (mvuData as any)?.主角?.行动预判 ?? (mvuData as any)?.行动预判;
  })();
  ...
  // 关键：不要使用 fallbackActions。
  // 这样当独立行动预判世界书关闭（或正确 remove 后）时，行动预判面板应为空；
  // 当世界书确实提供了 /主角/行动预判/选项 数组时，actions 会非空并正常渲染。
  // 最终兜底：没有选项时也必须显示“现在可自由行动”
  ...
  const pushActionToTavern = (action: ActionOption) => {
    ...
    const g = globalThis as any;
    // 重点：主酒馆输入框通常在父页面；优先使用 window.parent.$
    const jqParent = (window.parent as any)?.$;
    const jq = typeof jqParent === 'function' ? jqParent : typeof g.$ === 'function' ? g.$ : undefined;

    const trySetInTextarea = (rootJq: any) => {
      const input = rootJq(
    ...
    if (typeof jq === 'function') {
      if (trySetInTextarea(jq)) {
        setIsActionPanelOpen(false);
        return;
      }
      // 再退一步：抓可见的 textarea（极少误命中，但移动端兼容性更强）
      const anyTextarea = jq('textarea').filter((_, el) => jq(el).is(':visible')).first();
      if (anyTextarea && anyTextarea.length) {
        const textarea = anyTextarea[0] as HTMLTextAreaElement;
        const prev = String(textarea?.value ?? '');
        const joined = prev.trim().length > 0 ? `${prev}\n${content}` : content;
        textarea.value = joined;
        textarea.dispatchEvent(new Event('input', { bubbles: true }));
          ...
          <div style={{ width: '100%' }}>
            <div className="hero-profile">
              ...
              <div className="hero-info">
                <h2>{hero.name}</h2>
                <div
                  className="hunter-level-pill"
                  style={
                    {
                      borderColor: hunterTheme.accent,
                      boxShadow: `0 0 18px ${hunterTheme.glow}`,
                      background: hunterTheme.gradient,
                    } as any
                  }
                >
                  <span className="hunter-level-icon" aria-hidden style={{ color: hunterTheme.accent } as any}>
                    <span className="hunter-level-icon-glyph">✦</span>
                  </span>
                  <span className="hunter-level-label">猎人</span>
                  <span className="hunter-level-value">{hunterLevel}</span>
                </div>
              </div>
            </div>
            {styleSelectorOpen && (
                  ...
                  <div className="style-selector-grid">
                    {DMC_STYLES.map(s => (
                      <button
                        ...
                        title={`风格进度：${getStyleProgress(s.cn)}%`}
                        onClick={() => {
                          const displayCn = s.cn;
                          void pushActionToTavern({
                            label: displayCn,
                      ...
                      >
                        <div
                          className="style-option-progress-bar"
                          aria-hidden="true"
                          style={
                            {
                              '--progress-fill': `${getStyleProgress(s.cn)}%`,
                            } as any
                          }
                        >
                          <div
                            className="style-option-progress-fill"
                            style={{ background: getStyleProgressGradientVertical(s.cn) }}
                          />
                        </div>
                        ...
                        <span className="style-option-pct">{getStyleProgress(s.cn)}%</span>
                      </button>
                    ))}
                  </div>
      ...
      <div className={`action-panel ${isActionPanelOpen ? 'active' : ''}`} onClick={() => setIsActionPanelOpen(false)}>
        <div className="action-panel-content" onClick={e => e.stopPropagation()}>
          ...
          <div className="action-list">
              ...
              const chanceTone = getChanceTone(chanceVal);
              return (
                <div
                  ...
                  onClick={() => void pushActionToTavern(action)}
                >
                  ...
                  >
                    <span className="chance-val">
                      {`${Math.max(0, Math.min(100, chanceVal))}%`}
                    </span>
